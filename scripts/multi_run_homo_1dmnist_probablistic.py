...
Description = f"Homo_{act_string}_1dmnist_probablistic_strat_{optimizer.__name__}_no_bias_min_{min_neurons}_{hidden_sizes[0]}_{hidden_sizes[1]}_{num_epochs}_{intervene_every}_{threshold}_runs_{NUM_RUNS}"
...
for run in range(NUM_RUNS):
    ...
    for epoch in range(num_epochs):
        ...
        if ((epoch + 1) % intervene_every*int(n_neurons/init_neurons) == 0 # scale intervention period linearly with number of neurons
            ...
            # if relative difference in test loss is positive and high, stop training
            if (test_loss - check_loss)/check_loss > 0.1 and epoch > 0:
                logging.info(f"Test loss increased by more than 0.1, stopping training at epoch {epoch}")
    ...
    np.savetxt(f"{run_output_folder}/accuracy.txt", accuracy_history)
