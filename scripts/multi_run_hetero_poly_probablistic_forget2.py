...
Description = f"Hetero_{act_string}_poly_normalized_probablistic_forget2_strat_{optimizer.__name__}_no_bias_min_{min_neurons}_{hidden_sizes[0]}_{hidden_sizes[1]}_{num_epochs}_{intervene_every}_{threshold}_runs_{NUM_RUNS}"
...
for run in range(NUM_RUNS):
    ...
    for epoch in range(num_epochs):
        ...
        if ((epoch + 1) % intervene_every*int(n_neurons/init_neurons) == 0 # scale intervention period linearly with number of neurons
            ...
            normed_loss = test_loss / L_0
            add_prob = p_add(normed_loss, n_neurons, min_neurons)/p_norm(normed_loss, n_neurons, min_neurons)
            add_prob_history.append((epoch, add_prob))
            remove_prob = p_remove(normed_loss, n_neurons, min_neurons)/p_norm(normed_loss, n_neurons, min_neurons)
            remove_prob_history.append((epoch, remove_prob))
            continue_prob = p_continue(normed_loss, n_neurons, min_neurons)/p_norm(normed_loss, n_neurons, min_neurons)
            layers = len(mlp.get_shape()) - 1
            
            # Neuron Removal criteria
            if jax.random.uniform(prob_key, minval = 0, maxval = 1)<remove_prob:
                if ((check_loss < test_loss) # check current loss against last accepted addition
                    ...
                    if len(mlp.layers[layer]) <= 1: # if only one neuron in layer, remove from another layer
                        layer_num = jax.random.randint(layer_key, (), 0, layers-1)
                        layer = jnp.where(layer_num >= layer, layer_num + 1, layer_num)
                        ...
                        logging.info(f"Removed neuron to hidden layer {layer+1} at index {neuron_idx} instead")
            ...
            if jax.random.uniform(prob_key, minval = 0, maxval = 1)< add_prob:
                ...
                logging.info(f"Check loss updated to {check_loss}")
    ...
    np.savetxt(f"{run_output_folder}/add_prob.txt", add_prob_history)
    np.savetxt(f"{run_output_folder}/remove_prob.txt", remove_prob_history)
