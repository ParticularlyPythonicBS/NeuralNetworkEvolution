...
Description = f"Hetero_{act_string}_poly_bare_probablistic_only_add_strat_{optimizer.__name__}_no_bias_min_{min_neurons}_{hidden_sizes[0]}_{hidden_sizes[1]}_{num_epochs}_{intervene_every}_{threshold}_runs_{NUM_RUNS}"
...
for run in range(NUM_RUNS):
    ...
    for epoch in range(num_epochs):
        ...
        if ((epoch + 1) % intervene_every*int(n_neurons/init_neurons) == 0 # scale intervention period linearly with number of neurons
            ...
            add_prob = p_add(test_loss, n_neurons, min_neurons)
            add_prob_history.append((epoch, add_prob))


            # Neuron Addition criteria
            # Neuron Addition criteria
            if jax.random.uniform(prob_key, minval = 0, maxval = 1)< add_prob: # addition probability
